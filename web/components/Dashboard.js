...
const Dashboard = () => {
    ...
    const sendNLCommand = async (customCmd) => {
        ...
        // Use AbortController for a generous timeout (60s) so long but valid signal requests
        // don't get killed by the browser and show the scary error.
        ...
        try {
            ...
            setChatLog(prev => {
                ...
                if (e.name === 'AbortError') {
                    return [...filtered, { id: "err-" + Date.now(), who: "bot", text: "⏳ The agent is still working on a complex analysis (this can take 10-30s on first queries). Please wait or try a simpler command like 'signals on BTC'." }];
                }
                    ...
                    <div className="bg-black/40 border border-white/5 rounded-xl p-4 h-[240px] overflow-y-auto flex flex-col gap-3 font-body text-xs">
                        {chatLog.map((msg) => (
                            <div 
                                ...
                                } ${msg.isSmall ? 'opacity-70 text-[10px]' : ''} ${msg.isThinking ? 'thinking' : ''}`}
                            >
