...
# ---------------------------------------------------------------------------
# STRATEGY_ID_MAP — canonical IDs used by the NL parser to select strategies
# ---------------------------------------------------------------------------
# Built lazily at class-definition time so instances are created only once.
_ALL_STRATEGY_INSTANCES: Optional[List["BaseStrategy"]] = None

def _build_all_strategies() -> List["BaseStrategy"]:
    """Return a fresh list of all 24 strategy instances."""
    return [
        EMACrossStrategy(),
        SMACrossStrategy(),
        SupertrendStrategy(),
...
# Mapping from short string IDs (used in NL parsing) to strategy class names.
# The IDs must match the .name attribute of each strategy.
...
class StrategySelector:
    def __init__(
        self,
        state_file_path: str = "strategy_performance.json",
        ...
        ----------
        state_file_path : str
            Path to the JSON file tracking historical strategy performance.
        strategy_ids : list[str] | None
            Optional subset of strategy IDs (keys of STRATEGY_ID_TO_NAME) to
            use.  When *None* (default) all 24 strategies are active, which
            preserves the existing behaviour.
        """
        self.state_file_path = state_file_path
        all_strats = _build_all_strategies()

        if strategy_ids:
            # Resolve IDs → canonical .name strings
            wanted_names: set = set()
            unknown_ids: List[str] = []
            for sid in strategy_ids:
                name = STRATEGY_ID_TO_NAME.get(sid)
                if name:
                    wanted_names.add(name)
                else:
                    unknown_ids.append(sid)
                    print(f"[StrategySelector] Unknown strategy ID '{sid}' — ignored.")

            filtered = [s for s in all_strats if s.name in wanted_names]
            if not filtered:
                # Graceful fallback: none of the requested IDs matched
                print(
                    "[StrategySelector] None of the requested strategy IDs matched "
                    "any known strategy — falling back to all 24 strategies."
                )
                filtered = all_strats

            self.strategies: List[BaseStrategy] = filtered
            self.using_subset: bool = True
            self.active_ids: List[str] = strategy_ids
        else:
            self.strategies = all_strats
    ...
    def _load_performance(self) -> Dict[str, Any]:
        if os.path.exists(self.state_file_path):
            try:
                ...
                print(f"[StrategySelector] Could not load performance state: {e}. Starting fresh.")
    ...
    def select_best_trade(
        ...
        confirmation_mode: str = "weighted",
    ) -> Dict[str, Any]:
        """
        Run strategies, detect regime, rank them, and return the winning setup.
        ...
        df : pd.DataFrame
            OHLCV candle data.
        style : str | None
            Trading style hint ('scalping', 'swing', or None).
        confirmation_mode : str
            ``'weighted'`` (default) — existing behaviour: weighted-average
            scoring across all active strategies.
            ``'unanimous'`` — all active strategies must agree (all BUY or all
            SELL); disagreement → HOLD with a detailed clash explanation.
        """
        if len(df) < 55:
            return {
                "symbol": "UNKNOWN",
        ...
        if not raw_signals:
            ...
            return {
                ...
                "score": 0.0,
                "reasoning": (
                    f"No strategies generated a trading signal. "
                    f"Market regime detected: {regime}. "
                    f"Active strategies: [{strat_names}]."
                )
            }

        # ── UNANIMOUS CONFIRMATION MODE ─────────────────────────────────────
        if confirmation_mode == "unanimous" and len(self.strategies) > 1:
            buys  = [r for r in raw_signals if r["signal"] == 1]
            sells = [r for r in raw_signals if r["signal"] == -1]
            # Strategies that fired (non-zero)
            fired = buys + sells
            ...
            if not fired:
                return {
                    ...
                    "score": 0.0,
                    "reasoning": (
                        "Unanimous mode: none of the selected strategies produced "
                        f"a signal this bar. Active: [{', '.join(s.name for s in self.strategies)}]. "
            ...
            # Check for clash
            if buys and sells:
                buy_names  = ", ".join(r["strategy"] for r in buys)
                sell_names = ", ".join(r["strategy"] for r in sells)
                return {
                    "decision": "HOLD",
                    "confidence": "LOW",
                    "strategy": "Unanimous (Clashed)",
                    "score": 0.0,
                    "reasoning": (
                        f"⚠️ Strategy conflict — unanimous agreement required but strategies disagreed. "
                        f"BUY signals: [{buy_names}]. "
                        f"SELL signals: [{sell_names}]. "
                        "No trade was opened. Adjust the strategy combination or wait for alignment."
                    ),
                    "regime": regime,
                    "regime_metrics": regime_metrics,
                    "ranked_opportunities": fired,
                }

            # All agree — pick highest-confidence signal as winner
            unanimous_signals = buys if buys else sells
            winner = max(unanimous_signals, key=lambda r: r["confidence"])
            agreed_names = ", ".join(r["strategy"] for r in unanimous_signals)
            decision_label = "BUY" if winner["signal"] == 1 else "SELL"
            confidence_label = (
                "HIGH" if winner["confidence"] >= 0.75
                else ("MEDIUM" if winner["confidence"] >= 0.5 else "LOW")
            )
            winner["score"] = round(winner["confidence"], 3)
            ...
            if style == "scalping":
                winner["stop_loss"] = round(
                    entry_price - (entry_price - sl) * 0.5 if winner["signal"] == 1
                    else entry_price + (sl - entry_price) * 0.5, 4)
                winner["take_profit"] = round(
                    entry_price + (tp - entry_price) * 0.5 if winner["signal"] == 1
                    else entry_price - (entry_price - tp) * 0.5, 4)
            elif style == "swing":
                winner["stop_loss"] = round(
                    entry_price - (entry_price - sl) * 1.5 if winner["signal"] == 1
                    else entry_price + (sl - entry_price) * 1.5, 4)
                winner["take_profit"] = round(
                    entry_price + (tp - entry_price) * 1.5 if winner["signal"] == 1
                    else entry_price - (entry_price - tp) * 1.5, 4)

            return {
                "decision": (
                    "STRONG BUY" if decision_label == "BUY" and confidence_label == "HIGH"
                    else ("STRONG SELL" if decision_label == "SELL" and confidence_label == "HIGH"
                          else decision_label)
                ),
                "confidence": confidence_label,
                "strategy": agreed_names,
                "score": winner["score"],
                "entry_price": winner["entry_price"],
                "stop_loss": winner["stop_loss"],
                "take_profit": winner["take_profit"],
                "reasoning": (
                    f"✅ Unanimous agreement across [{agreed_names}]: all signal {decision_label}. "
                    f"Highest-confidence strategy: '{winner['strategy']}' ({winner['confidence']:.0%}). "
                    f"{winner['reasoning']} Market regime: {regime}."
                ),
                "regime": regime,
                "regime_metrics": regime_metrics,
                "ranked_opportunities": unanimous_signals,
            }
        # ── END UNANIMOUS MODE ───────────────────────────────────────────────
