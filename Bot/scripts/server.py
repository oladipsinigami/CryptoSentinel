...
def _get_live_signals(symbol="BTCUSDT", style=None):
    ...
    try:
        ...
        try:
            if df_ohlcv is not None and not df_ohlcv.empty:
                ...
                strategy_intelligence = selector.select_best_trade(df_ohlcv, style=style)
                if strategy_intelligence.get("strategy") and strategy_intelligence.get("strategy") != "None":
                    strategy_note = f"**Strategy Layer**: {strategy_intelligence['strategy']} ({strategy_intelligence['decision']} - {strategy_intelligence['confidence']}) | Regime: {strategy_intelligence['regime']}"
        except Exception as e:
...
def _parse_trade_intent(text: str) -> dict:
    ...
    # Detect trade/execute style requests (e.g. "scalp trade", "take a scalp trade", etc.)
        ...
        if has_trade_action and not is_info_seeking:
            action = "execute_trade"
            if any(w in t for w in ["long", "buy", "bullish"]):
                side = "LONG"
            elif any(w in t for w in ["short", "sell", "bearish"]):
    ...
    use_best = False
    if any(kw in t for kw in ["or other coins", "best", "strongest", "top", "most profitable", "autonomous", "the best one", "best coin"]):
        use_best = True
...
def _execute_intelligent_trade(intent: dict, user_text: str, resolved_prices: Optional[dict] = None):
    ...
    if resolved_prices:
        side = resolved_prices.get("direction", "LONG")
    ...
    if ok:
        return f"✅ Executed {action} {symbol} ${amount:.0f} @ ${price:,.2f} | SL ${sl} TP ${tp} ({lev}x)\nReason: {user_text}"
    else:
        return f"Trade attempt failed: {err[:150] or 'see logs'}"


def _generate_formatted_signal(sym, style=None):
    """
    ...
    """
    try:
        data, msg = _get_live_signals(sym, style=style)
    except Exception as e:
...
def handle_nl_command(command_text: str):
    ...
    # 2. SCAN_BEST action
    if action == "scan_best":
        is_execution = any(w in text.lower() for w in ["trade for me", "execute", "place", "buy", "sell", "long", "short"]) and not any(w in text.lower() for w in ["give me", "get me", "show me", "what is", "analyze", "analysis"])
        if is_execution:
    ...
    if action == "execute_trade":
        ...
        if use_best:
            ...
            data, sig_msg = _generate_formatted_signal(sym, style=style)
            if not data:
                return {
                    "ok": False,
                    "message": sig_msg,
    ...
    if action == "signal":
        sym = symbol if symbol else "BTCUSDT"
        print(f"[NL] SIGNAL INTENT DETECTED → fetching for {sym} (style: {style})")
        data, msg = _generate_formatted_signal(sym, style=style)
        return {"ok": data is not None, "message": msg, "action": "signals", "signals": data}

    # === STATUS / PORTFOLIO ===
    if any(kw in tlower for kw in ["portfolio", "balance", "pnl", "status", "how am i", "how are we",
                                    "open position", "my position", "my money", "my trades", "my balance",
                                     "current balance", "what are my"]):
        try:
            with open(STATE_FILE) as f:
                st = json.load(f)
    ...
    # === SCAN / TOP MOVERS ===
    if ("scan" in tlower or "top conviction" in tlower or "top movers" in tlower or "top coins" in tlower) and "trade" not in tlower:
        data, msg = _get_market_overview()
        return {"ok": True, "message": msg, "action": "overview", "data": data}
    ...
    # === HELP ===
    if "help" in tlower or "what can you" in tlower or "capabilities" in tlower:
        return {
            "ok": True,
            "message": (
                "I can do a lot more now:\n"
                "• 'give me signals on BTC' or 'analyze ETH' → live perception (technicals + official MCP sentiment, derivatives, fear&greed)\n"
                "• 'trade for me' / 'trade the best coin' / 'long SOL 150 3x' → paper trading\n"
                "• 'close my positions' or 'exit BTC'\n"
                "• 'what is the market sentiment' or 'long short ratio'\n"
                "• 'show my portfolio' / 'how am I doing'\n"
                "• 'scan the market' (analysis only)\n"
                "• 'run a backtest'\n"
                "• 'reset the portfolio'\n"
                "Type naturally — I understand symbols, amounts, leverage, trading styles (scalping, swing), and intent. I use the same Bitget Skill Hub MCP data + official kline indicators as the 5 official skills."
            ),
            "action": "help"
        }
